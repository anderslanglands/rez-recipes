...
def variants():
    ...
    if cook_variant:
        ...
        return [
            ["platform-linux", "arch-x86_64", "cxx11abi", "cfg", "python"],
            ["platform-windows", "arch-AMD64", "vs", "cfg", "python"],
        ]


def commands():
    env.MATERIALX_ROOT = "{root}"
    env.MATERIALX_ROOT_DIR = "{root}"
...
def pre_cook():
    download_and_unpack(
        f"https://github.com/AcademySoftwareFoundation/MaterialX/releases/download/v{version}/MaterialX-{version}.tar.gz"
    )
