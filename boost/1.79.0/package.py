...
def build_command():
    ...
    if platform.system() == "Windows":
        ...
        return f"cd $REZ_BUILD_SOURCE_PATH && ./bootstrap.sh && ./b2 install --prefix=$REZ_BUILD_INSTALL_PATH address-model=64 link=shared threading=multi --layout=system variant=$REZ_BUILD_CONFIG -j$REZ_BUILD_THREAD_COUNT -d0 cxxflags=$CXXFLAGS"